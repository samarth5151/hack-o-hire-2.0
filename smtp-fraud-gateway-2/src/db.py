...
def init_db():
    ...
    try:
        ...
        # Add columns if not already present (migration for existing DBs)
        new_cols = [
            ("source", "TEXT DEFAULT 'SMTP'"),
            ("ml_analysis", "TEXT"),
            ("ml_classification", "TEXT"),
...
def save_decision(sender, recipients, subject, fraud_score, decision,
                  ...
                  ml_analysis=None, ml_classification=None, ml_confidence=None,
                  detected_languages=None, explanation=None):
    c, be = _conn()
    try:
        cur = c.cursor()
        ...
        if be == "pg":
            cur.execute("""
                ...
                (sender, recipients, subject, fraud_score, decision, threat_type,
                 shap_values, features, top_contributors, processing_ms, raw_email, source,
                 ml_analysis, ml_classification, ml_confidence, detected_languages, explanation)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id
            """, (sender, recip, subject, fraud_score, decision, threat_type,
                  shap_s, feat_s, top_s, processing_ms, raw_email, source,
                  ml_s, ml_classification, ml_confidence, lang_s, explanation))
...
def _row_to_dict(row, json_fields=("shap_values", "features", "top_contributors", "recipients", "ml_analysis", "detected_languages")):
    d = dict(row)
...
def get_decisions(limit=50, decision_filter=None, source_filter=None):
    ...
    try:
        ...
        cols = ("id, created_at, sender, recipients, subject, fraud_score, "
                "decision, threat_type, shap_values, features, top_contributors, "
