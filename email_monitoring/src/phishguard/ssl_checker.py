"""
ssl_checker.py â€“ Checks SSL certificate validity for a domain.
"""
...
def check_ssl(domain: str) -> SSLResult:
    ...
    try:
        ...
        return _error_result(domain, "unreachable", "Domain does not resolve (DNS failure â€” likely a dead or fake domain)")
    ...
    try:
        ...
        return _error_result(domain, "invalid", "Certificate is UNTRUSTED â€” self-signed or fake CA")
    except ssl.SSLError as e:
        msg = str(e)
        short = msg[:80] if len(msg) > 80 else msg
        return _error_result(domain, "invalid", f"SSL handshake failed: {short}")
    except (socket.timeout, ConnectionRefusedError):
        return _error_result(domain, "no_ssl", "Port 443 refused â€” site may not support HTTPS")
    except Exception as e:
